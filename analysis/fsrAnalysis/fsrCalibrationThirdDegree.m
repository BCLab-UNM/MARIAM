...
filename = 'fsrCalibrationAverages_only1k.csv';
...
for i = 1:length(resistorNames)
    varName = resistorNames{i};
    plot(expVoltageOut.(varName), force, 'LineWidth', 2);%, 'Colorca', colors(i, :));
    legendTitles{end+1} = [strrep(varName(2:end), '_', '.') ' Ohm' ];
    hold on
end
hold off
...
coeffMatrix = zeros(length(resistorNames), 4);  % 6 coefficients for a 5th-degree polynomial
for i = 1:length(resistorNames)
