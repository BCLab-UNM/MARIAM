...
class ForceListener : public rclcpp::Node {
  public:
    ForceListener() : Node("force_listener") {
      ...
      force_sub = this->create_subscription<std_msgs::msg::Float32>(
        "force",
        qos_profile,
        std::bind(&ForceListener::force_callback, this, std::placeholders::_1)
      );
      ...
      auto timer_period = std::chrono::milliseconds(2);
      timer_ = this->create_wall_timer(
        timer_period,
        std::bind(&ForceListener::callback, this)
      );
      RCLCPP_INFO(this->get_logger(), "Force listener initialized.");
    }
    ...
    void write_to_csv() {
      ...
      try {
        ...
        if (trial_name.find('/') != std::string::npos || trial_name.front() == '~') {
          ...
          std::string filename = robot_name_str + "_position_data.csv";
          ...
          file_path = data_dir / "position_data.csv";
        }
      ...
      // Write header
      file << "force,ros_time_sec,ros_time_ns\n";
  ...
  private:
    ...
    const rclcpp::Logger LOGGER = rclcpp::get_logger("force_listener");
    ...
    void force_callback(const std_msgs::msg::Float32 &msg) {
...
int main(int argc, char * argv[]) {
  ...
  auto node = std::make_shared<ForceListener>();
  rclcpp::spin(node);
